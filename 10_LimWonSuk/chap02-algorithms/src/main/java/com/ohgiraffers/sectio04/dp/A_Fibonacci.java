...
public class A_Fibonacci {
    ...
    public static int getfibonacciNumberDP(int n) {
        if (n == 0) {
            ...
            if (DP [n] == 0) {
                // 2. 저장 (Store) : 계산되지 않은 값은 연산 후 특정 자룡 구조에 저장(Meoiztion)
    ...
    * - Tabulation(타블레이션)이용
    *   ㄴ 작은문제(부분문제)로 나눈 후 연산 결과를 테이블(배열)에 저장하는 방식
    * */

    public static int getFibonacciNumberIter(int n) { // n = 6 일때

        int[] arr = new int[n + 1]; // 칸을 7칸으로 만듬 -> 0번 인덱스 생성
        arr[0] = 0; // 0번 인덱스 칸에 대입
        arr[1] = 1; // 1번 인덱스 칸에 대입


        if (n == 0) return arr[0]; // 만약 n=0이 아니면 건너 뛰어라
        else if (n == 1) return arr[1];


        else {
            for (int i = 2; i < n + 1; i++) arr[i] =
                    // i = 2~6 을 넣는다
                    arr[i - 1] + arr[i - 2];
            // 2-1 + 2-2 = 1 값을  arr[i]에 대입해라 를  7번칸 까지하고 반복
            return arr[n];
        }
    }
