...
public class F_KruskalAlgorithm {
     ...
     */
    static class Edge implements Comparable<Edge> { // 가중치 기준 생성
        int u, v, weight;
         ...
         */
        @Override // compareTo를 오버라이딩 할때 가중치를 재설정하겠다
    ...
    public static Long solution(String input) throws IOException {
        ...
        for(Edge edge : edges) {
            ...
            if(find(edge.u) != find(edge.v)) { // input값에서 대입
                /* 사이클이 없으면 간선 선택 */

                /* 1. 두 정점을 하나의 집합으로 합치기 (연결) */
                union(edge.u, edge.v);  // 하나의 부모로 합쳐짐

                /* 2. 선택한 간선의 가중치를 총합에 추가 */
                totalWeight += edge.weight;

                /* 디버깅용 출력 (선택 사항) */
                 System.out.println("선택한 간선: " +
                         edge.u + "-" +
                         edge.v +
                         " (가중치: " + edge.weight + ")");
